...
public class UnoptimizedAlphanumericValidatorMicroBenchmark {
    ...
    public void isValid(Blackhole blackhole) {
        for (int count = 1; count <= totalBenchmarkIterations; count++) {
            boolean isValid = UnoptimizedAlphanumericValidator.instance.isValid(contentPassingAlphanumericValidation.get(count % totalContentItemsToBeValidated));
    ...
    public void isNotValid(Blackhole blackhole) {
        for (int count = 1; count <= totalBenchmarkIterations; count++) {
            boolean isValid = UnoptimizedAlphanumericValidator.instance.isValid(contentFailingAlphanumericValidation.get(count % totalContentItemsToBeValidated));
            blackhole.consume(isValid);
        }
    }
}